...
CREATE TABLE DIM_CUSTOMER AS (SELECT DISTINCT(CUSTOMER_ID) 
AS CUSTOMER_NUM FROM STG_ORDERS ORDER BY 1);
...
SELECT * FROM RFM2 LIMIT 10;
--------------------------------------------------
--SIXTH DAY CODE STARTS FROM HERE
---------------------------------------------------
SELECT * FROM STG_ORDERS LIMIT 5;
SELECT COUNT(*) FROM STG_ORDERS;
SELECT COUNT(DISTINCT(CUSTOMER_ID)) AS CUST_COUNT FROM STG_ORDERS;
SELECT T1.CUSTOMER_ID,T2.AMOUNT,T2.QUANTITY FROM DIM_CUSTOMER T1,STG_ORDERS T2
WHERE T1.CUSTOMER_NUM=T2.CUSTOMER_ID
LIMIT 5;

SELECT COUNT(DISTINCT(CUSTOMER_ID)) AS CUST_COUNT FROM STG_ORDERS;
SELECT T1.CUSTOMER_ID,T2.ORDER_ID,T2.AMOUNT,T2.QUANTITY FROM DIM_CUSTOMER T1,STG_ORDERS T2
WHERE T1.CUSTOMER_NUM=T2.CUSTOMER_ID
LIMIT 5;

SELECT COUNT(DISTINCT(CUSTOMER_ID)) AS CUST_COUNT FROM STG_ORDERS;
SELECT T1.CUSTOMER_ID,T2.ORDER_ID,T2.ORDER_DATE,T2.AMOUNT,T2.QUANTITY FROM DIM_CUSTOMER T1,STG_ORDERS T2
WHERE T1.CUSTOMER_NUM=T2.CUSTOMER_ID
LIMIT 5;

SELECT COUNT(DISTINCT(CUSTOMER_ID)) AS CUST_COUNT FROM STG_ORDERS;
SELECT T1.CUSTOMER_ID,T2.ORDER_ID,T3.TIME_ID,T2.AMOUNT,T2.QUANTITY 
FROM DIM_CUSTOMER T1,STG_ORDERS T2,DIM_TIME T3
WHERE T1.CUSTOMER_NUM=T2.CUSTOMER_ID AND T3.ORDER_DATE=T2.ORDER_DATE
LIMIT 5;

SELECT * FROM DIM_TIME LIMIT 5;
DROP TABLE FACT1 IF EXISTS;
create table FACT1 as (SELECT T1.CUSTOMER_ID,T2.ORDER_ID,T3.TIME_ID,T2.AMOUNT,T2.QUANTITY
FROM DIM_CUSTOMER T1,STG_ORDERS T2,DIM_TIME T3
WHERE T1.CUSTOMER_NUM=T2.CUSTOMER_ID AND T3.ORDER_DATE=T2.ORDER_DATE);

SELECT * FROM FACT1;
SELECT COUNT(*) FROM FACT1;
--CREATE RACENCY TABLE
--GET MAX TIME ID FOR EACH CUSTOMER
SELECT CUSTOMER_ID,MAX(TIME_ID) AS MAX_DATE FROM FACT1 
GROUP BY CUSTOMER_ID
LIMIT 5;
SELECT MAX(TIME_ID) FROM FACT1 --1047
SELECT CUSTOMER_ID,1047-MAX(TIME_ID) AS RACENCY FROM FACT1 
GROUP BY CUSTOMER_ID
LIMIT 5;

DROP TABLE RACENCY_TABLE IF EXISTS;
CREATE TABLE RACENCY_TABLE AS  (SELECT CUSTOMER_ID,1047-MAX(TIME_ID) AS RACENCY FROM FACT1 
GROUP BY CUSTOMER_ID);
SELECT * FROM RACENCY_TABLE ORDER BY 1  LIMIT 5;
SELECT COUNT(*) FROM RACENCY_TABLE LIMIT 5;

--CREATE FREQUENCY TABLE
SELECT DISTINCT(ORDER_ID)FROM FACT1 LIMIT 5;
SELECT CUSTOMER_ID ,COUNT(ORDER_ID) AS FREQUENCY FROM FACT1 
GROUP BY CUSTOMER_ID 
LIMIT 5;

DROP TABLE FREQUENCY_TABLE IF EXISTS;
CREATE TABLE FREQUENCY_TABLE AS SELECT CUSTOMER_ID ,
COUNT(ORDER_ID) AS FREQUENCY FROM FACT1 
GROUP BY CUSTOMER_ID;

SELECT * FROM FREQUENCY_TABLE ORDER BY 1 LIMIT 5;
SELECT COUNT(*) FROM FREQUENCY_TABLE;

--CREATE MONETARY TABLE
SELECT SUM(AMOUNT) FROM FACT1
SELECT CUSTOMER_ID ,SUM(AMOUNT) AS MONETARY FROM FACT1 
GROUP BY CUSTOMER_ID 
LIMIT 5;

DROP TABLE IF EXISTS MONETARY_TABLE;
CREATE TABLE MONETARY_TABLE AS (SELECT CUSTOMER_ID ,SUM(AMOUNT) AS MONETARY FROM FACT1 
GROUP BY CUSTOMER_ID);

SELECT COUNT(* ) FROM MONETARY_TABLE;
SELECT * FROM MONETARY_TABLE LIMIT 5;

--CREATE RFM TABLE
SELECT T1.CUSTOMER_ID,T1.RACENCY,T2.FREQUENCY,T3.MONETARY 
FROM RACENCY_TABLE T1,FREQUENCY_TABLE T2,MONETARY_TABLE T3
WHERE T1.CUSTOMER_ID=T2.CUSTOMER_ID AND 
T1.CUSTOMER_ID=T3.CUSTOMER_ID
LIMIT 5;

DROP TABLE  IF EXISTS RFM_TABLE;
CREATE TABLE RFM_TABLE AS (SELECT T1.CUSTOMER_ID,T1.RACENCY,T2.FREQUENCY,T3.MONETARY 
FROM RACENCY_TABLE T1,FREQUENCY_TABLE T2,MONETARY_TABLE T3
WHERE T1.CUSTOMER_ID=T2.CUSTOMER_ID AND 
T1.CUSTOMER_ID=T3.CUSTOMER_ID);

SELECT COUNT(*) FROM RFM_TABLE;
SELECT * FROM RFM_TABLE ORDER BY 1 LIMIT 5;
