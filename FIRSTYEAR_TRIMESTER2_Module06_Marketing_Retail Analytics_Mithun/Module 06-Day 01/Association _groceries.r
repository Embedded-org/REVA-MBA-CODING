...
plot(meat.rules,method="graph",interactive=TRUE,shading="lift")
title(main = "Meat")
...
title(main = "Coke")
trans.sel<-transactions[,itemFrequency(transactions)>0.1] # selected transactions dissimilarity(trans.sel, which="items")
...
plot(meat.rules, method="matrix", measure=c("support","confidence"), control=list(col=sequential_hcl(200)))
plot(meat.rules, method="grouped", measure="support", control=list(col=sequential_hcl(100)))
plot(meat.rules, method="paracoord", control=list(reorder=TRUE))
